...
use \app\resources\Resource_Model;
use \Flight;

class Resource_Controller{
   private Resource_Model $model;
   ...
   */
   private function resourceHandler(string $method, string $id) : void{
      switch($method){
         case "GET":
            echo $this->model->getResource($id);
         break;
         ...
         case "PATCH":
            if($this->updateValidation()){
               ...
               echo json_encode(array('status' => 'error', "msg" => "At least one of the following parameters must be passed: " . implode(", ", $this->keys)));
            }
         break;
